                ...
                onClick={() => {
                  ...
                  if (printWindow) {
                    printWindow.document.write(`
                      <html>
                        <head>
                          <title>Portfolio - ${account.name}</title>
                          <style>
                            body { font-family: system-ui, -apple-system, sans-serif; max-width: 800px; margin: 0 auto; padding: 40px 20px; color: #1a1a1a; }
                            h1 { font-size: 28px; margin-bottom: 8px; }
                            h2 { font-size: 18px; margin-top: 24px; margin-bottom: 12px; border-bottom: 2px solid #e2382b; padding-bottom: 8px; }
                            .meta { color: #666; font-size: 14px; margin-bottom: 24px; }
                            .section { margin-bottom: 24px; }
                            .item { margin-bottom: 12px; padding: 12px; background: #f9f9f9; border-radius: 8px; }
                            .item-title { font-weight: 600; margin-bottom: 4px; }
                            .item-subtitle { color: #666; font-size: 13px; }
                            .item-date { color: #999; font-size: 12px; margin-top: 4px; }
                            @media print { body { padding: 0; } }
                          </style>
                        </head>
                        <body>
                          <h1>${account.name}</h1>
                          <div class="meta">${account.location} · ${account.grade} · ${account.bio || ""}</div>
                            ...
                            ${sectionOrder.map(section => {
                              ...
                              return `
                                <div style="margin-bottom: 20px;">
                                  <h3 style="font-size: 16px; margin-bottom: 8px; color: #e2382b;">${sectionLabels[section]}</h3>
                                  ${items.map(item => `
                                    <div class="item">
                                      <div class="item-title">${item.title}</div>
                                      ${item.subtitle ? `<div class="item-subtitle">${item.subtitle}</div>` : ""}
                                      ${item.date ? `<div class="item-date">${item.date}</div>` : ""}
                                      ${item.description ? `<div style="margin-top: 6px; font-size: 13px; color: #444;">${item.description}</div>` : ""}
                                    </div>
                                  `).join("")}
                                </div>
                              `;
                          ...
                          <div style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ddd; font-size: 11px; color: #999;">
                            Generated by ULYS · ${new Date().toLocaleDateString("ru-RU")}
                          </div>
                        </body>
                      </html>
                    `);
              ...
              <button
                onClick={() => {
                   const exportData = {
                     ...
                     goals: account.goals,
                     portfolioStrength: account.portfolioStrength,
                     entries: entries,
                  ...
                  a.href = url;
                  a.download = `portfolio-${account.name.replace(/\s+/g, "-").toLowerCase()}.json`;
                  a.click();
                  URL.revokeObjectURL(url);
                  showToast("Портфолио экспортировано в JSON");
                }}
